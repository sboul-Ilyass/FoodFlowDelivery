...
import { Store, MapPin, UtensilsCrossed, ArrowLeft } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/customer/restaurant/$merchantId")({
...
function RestaurantMenu() {
  ...
  // Available products for this merchant
  ...
  return (
    <AppShell title={merchant?.name ?? "Menu"}>
      <div className="max-w-4xl space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 p-4 bg-card border rounded-xl">
            <div className="h-14 w-14 rounded-xl bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center shrink-0">
              <Store className="h-7 w-7 text-orange-500" />
            </div>
            <div>
              <h1 className="font-bold text-xl leading-tight">{merchant?.name ?? "Restaurant"}</h1>
        ...
        {products?.length === 0 && (
          <div className="py-20 text-center text-muted-foreground text-sm">
            No items available at this restaurant right now.
          </div>
        )}
